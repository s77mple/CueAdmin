...
function addChild() {
  form.children.push(childDefault());
}
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.id) {
      ...
      const res = await updateMenu(form.id, data);
      if (res.code !== 0) {
    ...
    for (const child of form.children ?? []) {
      if (!child.code?.trim()) {
        ElMessage.error("子菜单编码不能为空");
        return;
      }
      if (!child.name?.trim()) {
        ElMessage.error("子菜单名称不能为空");
        return;
      }
    }
    ...
    const parentRes = await createMenu(parentData);
    if (parentRes.code !== 0) {
    ...
    for (const child of form.children) {
      ...
      try {
        const childRes = await createMenu(childData);
    ...
    if (total === 0) ElMessage.success("创建成功");
      ...
      ElMessage.success(`创建成功，含 ${total} 个子菜单`);
    else
      ElMessage.warning(
        `父菜单已创建，但 ${failedCount}/${total} 个子菜单创建失败，请手动补建`
      );
