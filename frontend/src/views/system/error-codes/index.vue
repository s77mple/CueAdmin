...
import { getErrorCodes } from "@/api/system/meta";
import type { ErrorCodeItem } from "@/api/system/types";

const loading = ref(false);
const list = ref<ErrorCodeItem[]>([]);
...
const treeData = computed(() =>
  SEGMENTS.map(seg => ({
    ...
    _isGroup: true,
    _label: seg.label,
    _count: list.value.filter(i => i.code >= seg.min && i.code <= seg.max)
      .length,
    children: list.value
    ...
    >
      <template #title>
        <el-tag type="info" effect="plain"
          >共 {{ list.length }} 个错误码 · 数据来源 GET
          /api/v1/system/meta/error-codes</el-tag
        >
      </template>
