...
from app.schemas.post import PostCreate, PostItem, PostUpdate
...
class PostService:
    ...
    async def list_posts(self, page: int = 1, page_size: int = 100) -> PageData[PostItem]:
        ...
        return await self.post_repo.list_posts(page, page_size)
    ...
    async def create_post(self, body: PostCreate) -> Post:
        """创建岗位 — 双重唯一性保护。"""
        if await self.post_repo.get_by_code(body.code):
    ...
    async def delete_post(self, post_id: int) -> str:
        ...
        user_count = await self.post_repo.count_users(post_id)
