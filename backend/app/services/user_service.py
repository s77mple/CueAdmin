...
class UserService:
    ...
    async def create_user(self, body) -> User:
        """创建用户 — 双重唯一性校验 + 外键验证。"""
        # 应用层唯一性检查
        if (await self.db.execute(select(User).where(User.username == body.username))).scalars().first():
    ...
    async def _validate_department(self, department_id: int | None):
        ...
        if department_id is not None:
            dept = (await self.db.execute(select(Department).where(Department.id == department_id))).scalars().first()
            if not dept:
