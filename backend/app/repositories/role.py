...
class RoleRepository(BaseRepository):
    ...
    async def get_with_relations(self, role_id: int) -> Role | None:
        ...
        result = await self.session.execute(
            select(Role).options(selectinload(Role.permissions), selectinload(Role.menus)).where(Role.id == role_id)
        )
        return result.scalars().first()

    async def list_roles(self, page: int = 1, page_size: int = 100) -> PageData:
        """分页返回角色（预加载权限和菜单）。角色数量少，默认一次返回全部。"""
        stmt = select(Role).options(selectinload(Role.permissions), selectinload(Role.menus)).order_by(Role.id.asc())
    ...
    async def list_all(self) -> list[Role]:
        ...
        """
        result = await self.session.execute(select(Role).order_by(Role.id.asc()))
    ...
    async def get_user_ids(self, role_id: int) -> list[int]:
        ...
        result = await self.session.execute(select(user_roles.c.user_id).where(user_roles.c.role_id == role_id))
        return result.scalars().all()
