...
前端登录的完整交互流程：
  - 用户在登录页输入用户名密码
  - POST /api/v1/auth/login → 成功返回 { access_token, user, permissions, roles }
  - 前端把 access_token 存 localStorage
  - 后续请求，axios 拦截器自动加 Authorization: Bearer <token>
  - 路由跳转时，vue-router 守卫调用 initRouter()，从 GET /api/v1/routes 拿动态路由（菜单不随登录下发）
  - 管理页面通过 v-perms="['user:list']" 控制按钮显隐
  - 登出 → POST /api/v1/auth/logout → token 加入 Redis 黑名单 + 清除权限缓存
"""
...
from app.core.cache_keys import blacklist_key, login_fail_key, perm_key, session_key
...
async def login(
    ...
    fail_key = login_fail_key(body.username)
    ...
    try:
        ...
        await redis_client.delete(perm_key(result.user.id))  # 清除旧权限缓存（确保登录后权限是最新的）
    except RedisError:
...
async def logout(
    ...
    try:
        if jti:
            await redis_client.setex(blacklist_key(jti), ttl, "1")  # access 加入黑名单
        if user_id:
            await redis_client.delete(perm_key(user_id))  # 清除权限缓存
        if session_id:
            await redis_client.delete(session_key(session_id))  # 撤销 refresh 会话
    except RedisError:
